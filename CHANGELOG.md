...
- Mark one photo per section as featured (a bigger, roughly 2×2 cell that the rest of the section flows around) via an indented `featured: true` line or the new ★ button. At most one per section; featuring a different photo clears the previous one. The bigger cell shows in Reading Mode too, since it's part of the gallery's content, not an editing affordance.
- Every per-item control (caption, section-insert, feature toggle) is now also reachable via a small, always-visible "⋯" icon in each photo's corner, not hover alone — hover-only controls are permanently unreachable on touch devices, which have no hover state. Tapping the icon toggles that photo's controls open; tapping elsewhere closes them. The gallery-level settings gear button is likewise faintly visible at all times instead of hidden until hover.
